    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#holderbox').append(data);
                $('#holderbox').fadeIn(200,fm.filesUpdated)   
            })
...
fm.filesUpdated = function () {
    ...
    $('#holderbox').promise().done(function(){
        $('#fmiconbar span').each(function(i,e) {
            ...
            if ( (t!=='all') && $('.fmi img[data-type='+t+']:visible').length == 0 ) {
                $('#fmiconbar span[data-type='+t+']').fadeOut(200)
            } else {
                $('#fmiconbar span[data-type='+t+'], #fmiconbar span[data-type="all"]').fadeIn(200)
            }
        });
        $('#fmiconbar span[data-type="all"]').click();
    })
}
...
$(document).ready(function($) {
    ...
    $(window.parent).resize(function(event) {
		...
		/*The following line resizes the link dialog to take the full width of the page when a link is clicked more than once.
		  The link dialog will still resize, but only upon resizing the window. Bug Number: 4147617*/
   ...
   $('#holderbox').on('click','.fmd', function () {
    ...
    // New folder
    // =====================================================================
	if(!!document.getElementById('fmdirbox')){
    ...
    // Upload
    // =====================================================================
    if(!!document.getElementById('fmuploadform')){
